...
class QueryAnalysis(dspy.Signature):
    ...
    recommended_sources: str = dspy.OutputField(description="Recommended MCP server names (comma-separated, choose from: wikidata, dbpedia, web-search, arxiv, news-api, finance, github, weather, llama-mcp). For factual/encyclopedic knowledge use 'wikidata' and 'dbpedia'. For stock data use 'finance'. For news use 'web-search'. For scientific research use 'arxiv' and 'wikidata'. For weather/forecasts/temperature use 'weather'.")
